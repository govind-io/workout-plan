...
function calculateBodyFatPercentageSI(waist, neck, height) {
    ...
    const bodyFatPercentage = 495 / (1.0324 - 0.19077 * logWaistNeck + 0.15456 * logHeight) - 450;
...
function addMeasurementToTable(measurement) {
    ...
    // Get body fat category and style
    const { category: bodyFatCategory, style: bodyFatStyle } = getBodyFatCategoryAndStyle(measurement.bodyFat);

    // Get BMI category and style
    const { category: bmiCategory, style: bmiStyle } = getBMICategoryAndStyle(measurement.bmi);
    ...
    row.innerHTML = `
        ...
        <td style="${bodyFatStyle}">${measurement.bodyFat}% (${bodyFatCategory})</td> 
         <td style="${bmiStyle}">${measurement.bmi.toFixed(2)} (${bmiCategory})</td> 
        <td>
            <button class="btn delete-btn" onclick="deleteMeasurement('${measurement.id}')">Delete</button>
        </td>
    `;
...
function updateChart(chart, measurementField) {
    chart.data.labels = measurements.map(measurement => new Date(measurement.date));

    // Update the dataset
    if (Array.isArray(measurementField)) {
        chart.data.datasets[0].data = measurements.map(measurement => {
            const total = measurementField.reduce((sum, field) => sum + parseFloat(measurement[field]), 0);
            return total / measurementField.length; // Average for bilateral measurements
        });
    } else {
        chart.data.datasets[0].data = measurements.map(measurement => parseFloat(measurement[measurementField]));
    }
